...
class RefreshButton(Button):
    def __init__(self):
        super().__init__(
            ...
            custom_id="refresh_registered",
            emoji=":refresh:1439928076195860613",
        )

    async def callback(self, interaction: discord.Interaction):
        cog = interaction.client.get_cog("RegisteredAccounts")
        if cog:
            await cog.handle_refresh(interaction)


class RegisteredAccounts(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        self.data_file = "registered_accounts.json"
        self.default_data = {
            "last_count": None,
            "last_update": None,
            "historical_counts": [],
    ...
    def cog_unload(self):
        self.monitor_registered.cancel()
    ...
    async def monitor_registered(self):
        try:
            channel = self.bot.get_channel(int(self.bot.config["registered_channel_id"]))
            if not channel:
                return
            entries = await self.fetch_open_data()
            if not entries:
                return
            current_count = entries[-1]["registered_accounts"]
            if current_count != self.last_count:
                await self.safe_channel_edit(channel, f"👤 {current_count:,} Accounts")
                self.last_count = current_count
                logger.info(
                    f"Updated channel name to {current_count:,} registered accounts. "
                    f"Minimum edit interval: {self.minimum_edit_interval.total_seconds()}s"
                )
        except Exception as e:
            logger.error(f"Error in registered accounts monitoring: {e}", exc_info=True)

    @monitor_registered.before_loop
    async def before_monitor_registered(self):
        await self.bot.wait_until_ready()

    @app_commands.command(
        name="users", description="Get the current number of registered Ente accounts"
    )
    ...
    @users.error
    async def users_error(self, interaction: discord.Interaction, error):
        ...
        try:
            ...
            if entries:
                ...
                self.data["historical_counts"] = [
                    {
                        ...
                        "count": e["registered_accounts"],
                    }
                    for e in entries
                ]

                current = entries[-1]
                current_count = current["registered_accounts"]

                change_text = ""
                if len(entries) >= 2:
                    previous_count = entries[-2]["registered_accounts"]
                ...
                if len(entries) >= 2:
                    ...
                    if months_diff >= 1:
                        total_increase = current_count - oldest["registered_accounts"]
                        avg_monthly = total_increase / months_diff
                        avg_monthly_text = f"**{avg_monthly:+,.0f}** accounts/month"

                accounts_embed = discord.Embed(
                    title="🦆 Ente Registered Accounts", color=0x54B689
                )
                ...
                await interaction.edit_original_response(embed=accounts_embed, view=view)
            else:
                await interaction.edit_original_response(
                    content="Failed to fetch the current registered account count. Please try again later."
                )
        except discord.NotFound:
...
async def setup(bot):
    await bot.add_cog(RegisteredAccounts(bot))
